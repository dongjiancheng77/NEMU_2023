...
static uintptr_t loader(PCB *pcb, const char *filename)
{
  // TODO();
  Elf_Ehdr elf_header ;
  size_t success = ramdisk_read(&elf_header, 0, sizeof(Elf_Ehdr)); 
  assert(*(uint32_t *)elf_header.e_ident == 0x464c457f);
  if (success) {
    for (size_t i = 0; i < elf_header.e_phnum; i++) {
      // printf("%d\n",i);
      Elf_Phdr phdr;
      success = ramdisk_read(&phdr, elf_header.e_phoff + i * sizeof(Elf_Phdr), sizeof(Elf_Phdr)); //phoff!
      if (success) {
        if (phdr.p_type == PT_LOAD) {
          success = ramdisk_read((void *)(phdr.p_vaddr), phdr.p_offset, phdr.p_memsz);
